...
{
    public class RoundRobin : IStagger
    {
        private const int  _quantum = 2;
        ...
        private int CurrentQuantum { get; set; } = 0;
        ...
        public RoundRobin(IEnumerable<IProcess> initial) : this()
        {
        ...
        private void HandleReadyQueue(WriteCallback log)
        {
            ...
            {
                log($"-----------------");
                log($"PID {Current.ID.ToString().PadLeft(4, '0')} has exceeded its quantum.");
                log($"");
                log($"PID {Current.ID.ToString().PadLeft(4, '0')} no longer owns the CPU and has been moved to READY queue.");
                log($"-----------------");
                log($"");
                Current.ArrivalTime = Clock;
                Ready.Add(Current);
                Current = GetNext(log);
                CurrentQuantum = _quantum;
            } 
            ...
            {
                log($"-----------------");
                log($"PID {Current.ID.ToString().PadLeft(4, '0')} still owns the CPU.");
                log($"-----------------");
                log($"");
            }
        ...
        {           
            ...
            CurrentQuantum = CurrentQuantum == 0 ? _quantum : CurrentQuantum - 1;
